<%-- 
    Document   : QueryPage
    Created on : Mar 17, 2023, 2:04:50 PM
    Author     : india
--%>
...
<section id="main-content">
    <div class="bg-image">
        <br><center><h1>Query Page</h1></center><br><br>
        <div class="table-container">
            <form id="myform" action="AdminQueryList">
                <button type="submit" onclick="submitForm()" class="btn btn-primary">Search</button>
                <table class="table table-striped">
                    ...
                    <tbody>
                        <% 
                            //get the feedback list object from session
                        ...
                        %>
                        <tr>
                            <td><%=feedback.getRefid()%></td>
                            <td><%=feedback.getName()%></td>
                            <td><%=feedback.getEmail()%></td>
                            <td><%=feedback.getSub()%></td>
                            <td><%=feedback.getMessage()%></td>
                            <td><%=feedback.getDateTime()%></td>
                            <td><a href="AdminReplyConf?refid=<%=feedback.getRefid()%>&user=<%=feedback.getName()%>
                                   &email=<%=feedback.getEmail()%>&subject=<%=feedback.getSub()%>
                                   &message=<%=feedback.getMessage()%>&dateTime=<%=feedback.getDateTime()%>
                                   " onclick="disableLink(this)">Reply</a></td>
                        </tr>
                        <%
                        } 
                        }else{
